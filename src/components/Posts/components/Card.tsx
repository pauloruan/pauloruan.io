    ...
    <Link href={`/blog/${post.slug}`}>
      <CardContainer className="h-full w-full rounded-md transition-all duration-300 ease-in-out hover:shadow-lg group hover:bg-secondary">
        <CardHeader className="p-2">
          <CardTitle className="h-6 max-w-max flex justify-start items-center gap-1 font-sans text-center truncate">
            <FiArrowUpRight className="h-4 w-4 text-sm font-medium" />
            {post.title}
          </CardTitle>
          <CardDescription className="text-sm font-sans truncate">
            {post.description}
          </CardDescription>
        </CardHeader>
